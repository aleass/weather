...
// 天气监控
func watch_weather(i int, info url_info) {
	//默认轮询监控,时间频率为60分钟
	var (
	...
	for {
		...
		println(i, tempStatus[i])
		println(i, weathereStatus)
		//发送大于一小时才发生或天气发生变化
		if now.Unix()-lastTime >= 3600 || tempStatus[i] != SkyconStatus[_realtime.Skycon]+res.Result.Minutely.Description {
			//发送
